...
| Chart code duplicated instead of using shared helpers | Low | Low | The design spec (`docs/superpowers/specs/2026-09-20-sales-dashboard-design.md`) calls for shared `line_chart`/`bar_chart` helper functions in `app.py`; the implementation plan chose inline, duplicated `px.bar()` calls instead. Only 2 call sites today, so a shared `bar_chart(df, value_col, label_col, title)` helper would be a small, low-risk refactor (no test impact) if ever needed.<br>&nbsp;&nbsp;— Approved by Rolland Lopez to defer; a fix can be implemented later, should the project require it. |
